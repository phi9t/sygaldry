...
func TestRunCommandEnvOverride(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("MY_OVERRIDE", "base")

	result, err := RunCommand(context.Background(), RunCommandInput{
		Command:    "bash",
		Args:       []string{"-c", "echo $MY_OVERRIDE"},
		Env:        map[string]string{"MY_OVERRIDE": "override"},
		WorkflowID: "test-wf",
		StepID:     "env-override-step",
		LogDir:     dir,
	})
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(result.Stdout, "override") {
		t.Fatalf("stdout = %q, expected override value", result.Stdout)
	}
	if strings.Contains(result.Stdout, "base") {
		t.Fatalf("stdout should not contain base value: %q", result.Stdout)
	}
}
