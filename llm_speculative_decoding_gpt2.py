...
class SpeculationMetrics:
    ...
    def acceptance_rate(self) -> float:
        ...
        if self.total_speculated_tokens == 0:
            return 0.0
        return (
            self.total_accepted_tokens / self.total_speculated_tokens
            if self.total_speculated_tokens > 0
            else 0.0
        )
    ...
    def speedup_ratio(self) -> float:
        ...
        if self.acceptance_rate == 0 or self.total_speculation_rounds == 0:
            return 1.0
        avg_k = self.total_speculated_tokens / self.total_speculation_rounds
        return 1 + (self.acceptance_rate * max(avg_k - 1.0, 0.0))
    ...
    def to_dict(self) -> Dict[str, float]:
        ...
        return {
            ...
            "total_speculated_tokens": self.total_speculated_tokens,
...
class SpeculativeDecoder:
    ...
    def _generate_tokens(
        ...
        while tokens_generated < max_new_tokens:
            ...
            try:
                ...
                draft_tokens = self._draft_phase(
                    generated, speculation_length, temperature, top_p, top_k
                )
                ...
                self.metrics.total_speculated_tokens += len(draft_tokens)
    ...
    def _sample_token_id(
        ...
        if temperature == 1.0 and top_p == 1.0 and top_k == 0:
    ...
    def _draft_phase(
        self, context: List[int], k: int, temperature: float, top_p: float, top_k: int
    ) -> List[int]:
        """
        ...
        try:
            for step in range(k):
                with torch.no_grad():
                    ...
                    next_token_id = self._sample_token_id(
                        logits[0], temperature, top_p, top_k
                    )
    ...
    def _verification_phase(
        ...
        try:
            ...
            for i, draft_token in enumerate(draft_tokens):
                ...
                target_prediction = self._sample_token_id(
                    target_logits[0, position - 1], temperature, top_p, top_k
                )
