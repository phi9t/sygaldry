...
Options:
  ...
  --snapshot-ref <ref>      Digest-pinned snapshot image for full smoke tests.
...
Notes:
- full smoke mode requires Docker + NVIDIA runtime and a valid --snapshot-ref.
- packaged tarball includes provenance.json and SHA256SUMS.
USAGE
}
...
if command -v shellcheck >/dev/null 2>&1; then
    shellcheck -s bash -S warning "${SKILL_ROOT}/scripts/"*.sh
else
    log "shellcheck not found; skipping shellcheck"
fi
"${SKILL_ROOT}/scripts/validate_hermetic_infra.sh" "${SKILL_ROOT}"
...
run_local_runtime_smoke() {
    ...
    local temp_code_home
    temp_code_home="$(mktemp -d)"
    local temp_target
    temp_target="$(mktemp -d)"
    local skill_runtime="${temp_code_home}/skills/${SKILL_NAME}"

    mkdir -p "${temp_code_home}/skills"
    cp -a "${SKILL_ROOT}" "${skill_runtime}"

    (
        cd /tmp
        CODEX_HOME="${temp_code_home}" bash -lc '
            SKILL_DIR="${CODEX_HOME}/skills/zephyr"
            "${SKILL_DIR}/scripts/zephyr_vendor_infra.sh" --help >/dev/null
        '
    )

    mkdir -p "${temp_target}/repo"
    local kit_root
    kit_root="${temp_target}/repo/.sygal""dry/zephyr"
    "${skill_runtime}/scripts/zephyr_vendor_infra.sh" install \
        --target-repo "${temp_target}/repo" \
        --snapshot-ref "${smoke_snapshot_ref}" \
        --image-mode auto \
        --runtime-image smoke/zephyr:dev
    "${skill_runtime}/scripts/zephyr_vendor_infra.sh" check \
        --target-repo "${temp_target}/repo"

    "${kit_root}/bin/repoctl" config show >/dev/null
    "${kit_root}/bin/repoctl" verify image --skip-spack >/dev/null
}

run_full_container_smoke() {
    local smoke_snapshot_ref="$1"
    [[ -n "${smoke_snapshot_ref}" ]] || err "--snapshot-ref is required for --smoke-mode full"
    [[ "${smoke_snapshot_ref}" == *@sha256:* ]] || err "--snapshot-ref must be digest-pinned"
    require_cmd docker
    docker info >/dev/null 2>&1 || err "Docker daemon is not available"
    docker info 2>/dev/null | grep -qi nvidia || err "NVIDIA runtime not detected in docker info"
    ...
    log "Full smoke: repoctl verify image --skip-spack"
    "${kit_root}/bin/repoctl" verify image --skip-spack
    log "Full smoke: repoctl verify spack"
    "${kit_root}/bin/repoctl" verify spack
    log "Full smoke: repoctl verify uv-layering --no-gpu"
    "${kit_root}/bin/repoctl" verify uv-layering --no-gpu
}

log "Running runtime smoke checks"
run_local_runtime_smoke "${dummy_snapshot_ref}"
if [[ "${SMOKE_MODE}" == "full" ]]; then
    run_full_container_smoke "${SNAPSHOT_REF}"
else
...
builder_tool="skills/zephyr/scripts/package_infra.sh"
...
payload = {
    "schema_version": "zephyr-infra-provenance/v1",
    ...
    "runtime_contract": {
        ...
        "required_clis": ["bash", "docker", "git", "realpath", "awk", "sha256sum"],
    },
...
"${packaged_skill}/scripts/validate_hermetic_infra.sh" "${packaged_skill}"
artifact_target="$(mktemp -d)"
mkdir -p "${artifact_target}/repo"
"${packaged_skill}/scripts/zephyr_vendor_infra.sh" install \
    --target-repo "${artifact_target}/repo" \
    --snapshot-ref "${dummy_snapshot_ref}" \
    --image-mode auto \
    --runtime-image smoke/zephyr:dev
"${packaged_skill}/scripts/zephyr_vendor_infra.sh" check \
    --target-repo "${artifact_target}/repo"
