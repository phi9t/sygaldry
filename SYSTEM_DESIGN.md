...
**Version:** 2.2
**Date:** 2026-02-13
**Status:** High-level overview (non-authoritative for container infra contract)

---

## Scope Note

This document is a high-level architecture overview.

Authoritative design and consolidated content (compact, human-reviewable):
- `foundation.org` — pillars, system-at-a-glance, cross-cutting concerns, governance.

Authoritative infra contract (storage, launcher, snapshots):
- `container/ZEPHYR_SYSTEM_DESIGN.md`

When container storage layout, launcher defaults, snapshot policy, staging policy, or verification behavior is in question, follow `container/ZEPHYR_SYSTEM_DESIGN.md`. For engineering principles and merged design content, see `foundation.org`.
...
│                    (Any UID/GID)                            │
├─────────────────────────────────────────────────────────────┤
│  Shared Host Storage (Zephyr v2 layout)                     │
│  /mnt/data_infra/zephyr_container_infra/                    │
│  ├─ shared/               → shared caches                   │
│  ├─ sygaldry/             → builder Spack store             │
│  └─ projects/<project_id>/→ project home/workspace/outputs  │
├─────────────────────────────────────────────────────────────┤
│                   Docker Container                          │
...
- Authoritative Spack view design (options and container leverage): `foundation.org`, Pillar 1 (Spack View and P1.7)
...
- NVIDIA/CUDA pip packages excluded via UV override file

**Integration Strategy:**
- **Spack Packages (Locked)**: torch, torchvision, torchaudio, jax, jaxlib, triton, numpy, scipy, scikit-learn, numba, llvmlite, matplotlib, pandas, soundfile, jupyterlab (canonical list: `container/spack_owned_packages.conf`)
- **NVIDIA/CUDA (Excluded)**: All `nvidia-*` pip packages blocked via `container/nvidia_overrides.txt` (provided by Docker base image + Spack)
- **uv Packages (Free)**: transformers, vllm, sglang, datasets, requests, click, pydantic, development tools
- **Environment Variables**: `UV_NO_BUILD_ISOLATION_PACKAGE`, `UV_FIND_LINKS`
- **Detailed Design**: See `container/ZEPHYR_SYSTEM_DESIGN.md` "UV-Spack Package Layering" section
...
Docker Container
    ↓
Spack Dependencies (/opt/spack_store/view)
    ↓
Bazel Build System
    ↓
...
- Status: `/mnt/data_infra/zephyr_container_infra/projects/<project_id>/outputs/.run-metadata/<run_id>/<job>.status`
- Logs: `/mnt/data_infra/zephyr_container_infra/projects/<project_id>/outputs/.run-metadata/<run_id>/<job>-<timestamp>.jsonl`
...
**Core Features:**
- **Shared Host Storage**: Uses Zephyr v2 roots under `/mnt/data_infra/zephyr_container_infra/` (`shared/`, `sygaldry/`, `projects/<id>/`)
...
- **GPU Runtime Validation**: Requires NVIDIA runtime and validates host CUDA compatibility
- **Comprehensive Logging**: Detailed logging with caller information for debugging

**Workflow:**
1. **System Validation**: Checks Docker daemon and NVIDIA runtime availability
...
- `SYGALDRY_REQUIRED_CUDA_VERSION`: Minimum required host CUDA version
- `ZEPHYR_LEASE_MODE`, `ZEPHYR_CACHE_PROFILE`: Runtime isolation/cache behavior
