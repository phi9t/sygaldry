...
case "${SUBCMD}" in
  run)
    ...
    JSONL_FILE_CONTAINER="/workspace/outputs/.run-metadata/${RUN_ID}/${JOB_NAME}-${TS}.jsonl"
    STATUS_FILE_CONTAINER="/workspace/outputs/.run-metadata/${RUN_ID}/${JOB_NAME}.status"
