...
shellcheck -s bash -S warning bin/sygaldry container/launch_container.sh validate_all.sh 2>&1 | head -20
```
...
Update Status to "On Hold — blocked by <X>" and add a Blocked note in the index.
...
- For F: update the Status field to "On Hold — blocked by <X>"
...
- Missing test coverage for important functions
...
- RFC solution code snippets that reference logging APIs (`slog.Warn`) when the target file uses `workflow.GetLogger(ctx)` — always verify which logger the file already uses
...
- Blocked (F) RFCs are listed in the open table with an "On Hold" note
