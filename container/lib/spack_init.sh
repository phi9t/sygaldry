...
sygaldry_activate_env() {
    ...
    # Candidate order is intentional and canonical:
    #   1. SYGALDRY_SPACK_ENV (explicit override) — highest priority; respected unconditionally.
    if [[ -n "${SYGALDRY_SPACK_ENV:-}" ]]; then
        candidates+=("${SYGALDRY_SPACK_ENV}")
    fi

    #   2. Current directory — supports in-place development of a Spack env.
    if [[ -f "spack.yaml" ]] || [[ -f "spack.lock" ]]; then
        candidates+=(".")
    fi

    #   3. /opt/spack_env/default — baked snapshot image (spack_snapshot.dockerfile).
    #   4. /opt/spack_env/zephyr  — alternate baked name used by older snapshots.
    #   5. $SYGALDRY_ROOT/pkg/zephyr — workspace fallback when running from the sygaldry repo
    #      without a baked image (e.g., during local Spack builds).
    candidates+=(
