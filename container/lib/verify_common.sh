...
verify_run_no_gpu() {
    local image="$1"
    local cmd="$2"
    local _timeout="${SYGALDRY_VERIFY_TIMEOUT:-300}"
    timeout "${_timeout}" docker run --rm --entrypoint /bin/bash "${image}" -c "${cmd}"
}
...
verify_run_with_gpu() {
    ...
    local cmd="$2"
    local _timeout="${SYGALDRY_VERIFY_TIMEOUT:-300}"
    timeout "${_timeout}" docker run --rm --runtime=nvidia --gpus=all --entrypoint /bin/bash "${image}" -c "${cmd}"
}
